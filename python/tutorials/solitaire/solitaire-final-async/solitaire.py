...
class Solitaire(ft.Stack):
    ...
    async def restart_stock(self):
        while len(self.waste.pile) > 0:
            card = self.waste.get_top_card()
            await card.turn_face_down()
            await card.move_on_top()
            await card.place(self.stock)
